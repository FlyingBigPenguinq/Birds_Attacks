...
create: function()
{
	...
	powerUpIcon.scale.x = .7;
	powerUpIcon.scale.y = .7;
...
update: function() 
{
	...
	if (items.children.length == 0)
	{
		...
		if (protected == false && randomItem == 10
		   && timerCount < 17)
		{
...
function jumpPlayer() 
{
	...
	if (gameStarted == false)
	{
		...
		tween.onComplete.add(function () { instructions.destroy(); });
		tween.start();
...
function createEnemyBird() 
{
	...
	else if (timerCount >= 9)
	{
		...
		grumpy.dead = false; grumpy2.dead = false;
		grumpy3.dead = false;
		grumpy.r = 0; grumpy2.r = 0;
		grumpy3.r = 0;
...
function showLeftGrumpy()
{
	...
	grumpyComing.alpha = .6;
	enemiesComing.add(grumpyComing)

	// tween sprite
	var tween = game.add.tween(grumpyComing.scale).to( {x: .7, y:.7}, 300, Phaser.Easing.Linear.None, true, 0, 4, true);
}
...
function showRightGrumpy()
{
	...
	// tween sprite
	tween = game.add.tween(grumpyComing.scale).to( {x: .7, y:.7}, 300, Phaser.Easing.Linear.None, true, 0, 4, true);
}
