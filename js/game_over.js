...
{ 
	...
	game.add.tween(home).to({x: 160}, 						 		1000).easing(Phaser.Easing.Bounce.Out).start();
	game.add.tween(tryAgain).to({x: 270}, 						 		1000).easing(Phaser.Easing.Bounce.Out).start();
	...
	game.add.tween(nameLabel).to({y: 125}, 						 		1000).easing(Phaser.Easing.Bounce.Out).start();
	...
	game.add.tween(scoreLabel).to({y: game.world.centerY + 115}, 				 		1000).easing(Phaser.Easing.Bounce.Out).start();
	
	game.add.tween(muteButton).to({y: 460}, 				 		1000).easing(Phaser.Easing.Bounce.Out).start();
},
...
{
	buttonTouch.play();
	window.location.href = "http://mobgames.us";
	
	// open in a new window  (this might be blocked by popup blockers)  // window.open("http://www.jqueryfuns.com/xiaoyouxi", "_blank");
}
